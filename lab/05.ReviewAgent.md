...
3. **提交到 github**
   提交代码到本地，并 push 到远程仓库

#### 1.3 验证

- 在 github 上可以看到新创建的 branch

### 第二步：在 PR 中进行代码审查

#### 2.1 目标

使用 review agent 对 PR 进行代码审查

#### 2.2 操作步骤

1. **创建 PR 并指定 copilot 进行代码审查**
   点击按钮"request review from copilot""并创建 PR
   ![create pr](./images/051.png)

2. **检查审查结果**
   稍后即可看到 copilot 的审查结果
   ![review result](./images/052.png)

#### 2.3 验证

- 在 github 上可以看到审查结果 （注意：本 lab 不要进行合并 PR，该 PR 将在后续步骤继续使用）
...
- 使用 Review Agent 自动审查 Pull Request，生成审查报告并给出改进建议。

目标

- 自动运行静态检查与样式检查。
- 由 Agent 生成审查意见（安全、可维护性、测试覆盖等）。

前置条件

- 已配置 Review Agent 或集成工具（可选：GitHub Actions）。
- 被审查的 PR 示例。

步骤

1. 在 PR 触发点配置 Review Agent（或在本地运行审查脚本）。
2. 运行代码质量检查（lint、type check、单元测试）。
3. 让 Agent 生成审查评论并输出审查报告文件（e.g., review/report.md）。
4. 根据 Agent 建议修改并更新 PR。

产出 / 检查点

- review/report.md（审查摘要）
- 已应用的修复提交
- PR 审查记录

参考

- GitHub Review Agent / Actions 文档
